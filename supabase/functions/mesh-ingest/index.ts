// mesh-ingest: 42-node biometric mesh ingestion (26 EEG/biometric + 6 vascular + 10 kinetic)
//
...
interface NodeReading {
  node_id:              number                           // 1-42
  band_powers?:         Partial<Record<EegBand, number>> // EEG nodes (1-19)
  amplitude_uv?:        number                           // EEG nodes
  phase_deg?:           number                           // EEG nodes
  raw_value?:           number                           // biometric nodes (20-26)
  ...
  // Kinetic nodes (33-42)
  emg_uv?:              number                           // electromyography amplitude (µV)
  reflex_latency_ms?:   number                           // reflex arc transit time (ms)
  motor_intent_score?:  number                           // normalised motor planning 0-1
  ground_impedance_ohm?: number                          // foot-to-surface impedance (Ω)
}
...
async function freezeSnapshot(
  ...
  const stateRows = nodes.map(r => ({
    ...
    mean_amplitude:   r.amplitude_uv ?? r.raw_value ?? r.phase_shift_rad ?? r.emg_uv ?? null,
    std_amplitude:    0,
    ...
    emg_uv:                r.emg_uv              ?? null,
    reflex_latency_ms:     r.reflex_latency_ms   ?? null,
    motor_intent_score:    r.motor_intent_score  ?? null,
    ground_impedance_ohm:  r.ground_impedance_ohm ?? null,
  }))
...
Deno.serve(async (req) => {
  ...
  if (nodes.length > 42) {
    return new Response(JSON.stringify({ error: 'nodes array exceeds 42-node mesh limit' }), { status: 400 })
  }
  ...
  const readingRows = nodes.map(r => {
    const liveVal  = r.amplitude_uv ?? r.raw_value ?? r.phase_shift_rad ?? r.emg_uv ?? null
